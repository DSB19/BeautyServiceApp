...
<html>
<head>
<title>Pretty Please|Rejected Appointment</title>
...
<?php
$ret=mysqli_query($con,"select customer.FName,customer.LName,customer.Email,customer.Mb_Number,appoitment.ID as bid,appoitment.Apt_Number,appoitment.Apt_Date,appoitment.Apt_Time,appoitment.Message,appoitment.BookingDate,appoitment.Status from appoitment join customer on customer.ID=appoitment.Cust_ID where appoitment.Status='Rejected'");
$cnt=1;
while ($row=mysqli_fetch_array($ret)) {

?>

						 <tr> <th scope="row"><?php echo $cnt;?></th> <td><?php  echo $row['Apt_Number'];?></td> <td><?php  echo $row['FName'];?> <?php  echo $row['LName'];?></td><td><?php  echo $row['Mb_Number'];?></td><td><?php  echo $row['Apt_Date'];?></td> <td><?php  echo $row['Apt_Time'];?></td><?php if($row['Status']==""){ ?>
